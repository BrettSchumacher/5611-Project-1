...
void Model::loadMesh(unsigned int indMesh)
{
	...
	std::vector<Texture> textures = getTextures(JSON["materials"][matInd]);
...
std::vector<Texture> Model::getTextures(json material)
{
	...
	if (material.find("pbrMetallicRoughness") != material.end())
	{
		...
		{
			filePath = JSON["images"][(int)material["baseColorTexture"]["index"]]["uri"];
			//see if it's already loaded
			for (unsigned int i = 0; i < loadedTexName.size(); i++)
			{
				if (loadedTexName[i] == filePath)
				{
					//printf("Diffuse texture already loaded: %d\n", i);
			...
			if (!skip)
			{
				...
				Texture diffuse = Texture((fileDirectory + filePath).c_str(), "diffuse", loadedTex.size());
				textures.push_back(diffuse);
				loadedTex.push_back(diffuse);
				loadedTexName.push_back(filePath);
			}
		}

		if (material.find("metallicRoughnessTexture") != material.end())
		{
			filePath = JSON["images"][(int)material["metallicRoughnessTexture"]["index"]]["uri"];
			//see if it's already loaded
			for (unsigned int i = 0; i < loadedTexName.size(); i++)
			{
				if (loadedTexName[i] == filePath)
			...
			if (!skip)
			{
				...
				Texture specular = Texture((fileDirectory + filePath).c_str(), "specular", loadedTex.size());
	...
	*/
	...
	if (textures.size() == 0)
	{
		Texture diffuse = Texture(loadedTex.size());
		textures.push_back(diffuse);
		loadedTex.push_back(diffuse);
